<script lang="ts">
	...
	interface Props {
		...
		toggleFilters?: { id: string; name: string; description?: string; icon?: string }[];
	...
	let selectedValvesType = $state('tool');
						...
						{#each Object.keys(tools) as toolId}
							...
							>
								{#if !(tools[toolId]?.authenticated ?? true)}
									...
									<div class="absolute inset-0 opacity-50 rounded-xl cursor-not-allowed z-10"></div>
								{/if}
								<div class="flex-1 truncate">
									<div class="flex flex-1 gap-2 items-center">
