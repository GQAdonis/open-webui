<script lang="ts">
  ...
  interface Props {
    ...
    messageId: string;
    style?: 'default' | 'minimal' | 'icon-only';
    size?: 'sm' | 'md' | 'lg';
  }
  ...
  let hasArtifacts = $derived(messageContent ? hasArtifactInMessage(messageContent) : false);
  let artifactCount = $derived(hasArtifacts ? getArtifactCount() : 0);
  // Size classes
